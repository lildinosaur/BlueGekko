...
var adapter = baseConfig[baseConfig.persistAdapter];
const Persist = require(dirs.gekko + adapter.path + '/persist');
...
const GekkoManager = function () {
    ...
    var retrieveGekkos = this.persist.retrieve();
    this.archivedGekkos = retrieveGekkos.archivedGekkos;
    gekkos = retrieveGekkos.gekkos;
    modes = retrieveGekkos.modes;
    configs = retrieveGekkos.configs;
    ...
    for (var k in gekkos) {
        ...
        var state = gekkos[k];
        if (gekkos[k].active !== true) {
            continue;
        }
        if (modes[id] === undefined) {
            console.log('No Mode for gekko : ' + id);
            continue;
        }
        if (configs[id] === undefined) {
            console.log('No Config for gekko : ' + id);
            continue;
        }
        var mode = modes[id];
    ...
    console.log(`${now()} Loading Done !`);

    this.persisting = baseConfig.persist;
}
...
GekkoManager.prototype.restart = function(id) {
    ...
    console.log(`${now()} retarting Gekko ${id}`);
  
    this.gekkos[id] = this.archivedGekkos[id];
    this.gekkos[id].stopped = false;
    this.gekkos[id].active = true;
...
GekkoManager.prototype.saveGekkos = function (id) {
    if (this.persisting !== true) {
        return false;
    }

    this.persist.store(this.gekkos,this.archivedGekkos,this.modes,this.configs,id);

    broadcast({
        type: 'gekko_persisted',
        id
    });

    console.log(`${now()} ${id} persisted.`);
}
